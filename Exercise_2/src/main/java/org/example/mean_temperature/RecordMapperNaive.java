...
public class RecordMapperNaive extends Mapper<LongWritable, Text, Text, IntWritable> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        if (record.isMissingTemperature()) {
            context.getCounter(FaultyTemperatureCounter.MISSING).increment(1);
        }
        if(record.isMalformed()) {
            context.getCounter(FaultyTemperatureCounter.MALFORMED).increment(1);
        }
